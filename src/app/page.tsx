...
export default function Home() {
  ...
  const confirmDeleteJob = async () => {
    ...
    try {
      const res = await fetch(`/api/job/${jobToDelete.id}`, { method: "DELETE" });
      if (res.ok) {
        setCompletedScrapes(prev => prev.filter(j => j.id !== jobToDelete.id));
        setSelectedSourceJobs(prev => prev.filter(jId => jId !== jobToDelete.id));
        setJobToDelete(null);
      } else {
            ...
            {activeTab === "ANALYZE" && (
              <div className="p-4 bg-slate-50 rounded-lg space-y-4">
                ...
                <div className="space-y-4">
                  <div>
                    <h4 className="text-sm font-semibold text-slate-600 mb-2 px-1">Raw Scraped Data</h4>
                    <div className="max-h-40 overflow-y-auto border bg-white rounded-md p-2 space-y-1">
                      {completedScrapes.filter(s => !!s.rawScrapeData).length === 0 ? <p className="text-xs text-slate-400 p-2">No raw data available.</p> : null}
                      {completedScrapes.filter(s => !!s.rawScrapeData).map(scrape => (
                        <div key={`raw-${scrape.id}`} className="flex items-center justify-between p-2 hover:bg-slate-50 rounded group border border-transparent hover:border-slate-100">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-slate-600 mb-2 px-1">Previous Analysis Reports</h4>
                    <div className="max-h-40 overflow-y-auto border bg-white rounded-md p-2 space-y-1">
                      {completedScrapes.filter(s => !!s.resultReport).length === 0 ? <p className="text-xs text-slate-400 p-2">No reports available.</p> : null}
                      {completedScrapes.filter(s => !!s.resultReport).map(scrape => (
                        <div key={`report-${scrape.id}`} className="flex items-center justify-between p-2 hover:bg-slate-50 rounded group border border-transparent hover:border-slate-100">
                          <label className="flex items-center gap-3 cursor-pointer flex-1">
                            <input type="checkbox" checked={selectedSourceJobs.includes(scrape.id)} onChange={() => toggleSourceJob(scrape.id)} />
                            <span className="text-sm font-medium flex items-center gap-2 text-slate-700">
      ...
      {jobToDelete && (
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full flex flex-col overflow-hidden">
            <div className="p-4 border-b bg-red-50 text-red-700 flex items-center gap-2">
              <AlertTriangle size={20} />
              <h3 className="font-semibold text-lg">Confirm Deletion</h3>
            </div>
            <div className="p-6">
              <p className="text-slate-700 mb-2">Are you sure you want to delete this data? This action cannot be undone.</p>
              <p className="text-sm font-medium text-slate-900 bg-slate-100 p-2 rounded truncate">{jobToDelete.title}</p>
            </div>
